...
class ActorCriticNetCrawler(nn.Module):

    def __init__(self, state_size, action_size, noise):
        super(ActorCriticNetCrawler, self).__init__()
    ...
    def forward(self, state, action=None):
        ...
        x_a = F.leaky_relu(self.actor_fc1(state))
        x_a = F.leaky_relu(self.actor_fc2(x_a))
        x_a = F.leaky_relu(self.actor_fc3(x_a))
        ...
        x_c = F.leaky_relu(self.critic_fc1(state))
        x_c = F.leaky_relu(self.critic_fc2(x_c))
        x_c = F.leaky_relu(self.critic_fc3(x_c))
